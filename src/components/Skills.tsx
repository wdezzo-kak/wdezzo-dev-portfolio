...
const SkillItem = memo(({ skill }: { skill: Skill }) => {
  ...
  return (
    <div className="group relative p-6 bg-white dark:bg-black border-2 border-black dark:border-white hover:bg-black dark:hover:bg-white transition-all duration-150 flex flex-col items-center justify-center text-center aspect-square cursor-crosshair overflow-hidden">
      {Icon && (
        <Icon 
          size={48} 
    ...
    <section id="stack" className="py-20 bg-[#f0f0f0] dark:bg-neutral-900 border-b-4 border-black dark:border-white transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-none text-black dark:text-white">CORE<br/>STACK</h2>
            <p className="font-mono text-lg mt-4 md:mt-0 max-w-md border-l-4 border-brutal-orange-hot pl-4 text-black dark:text-neutral-300">The essential toolkit I use to craft structure, style, and interactivity.</p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 border-2 border-black dark:border-white">
