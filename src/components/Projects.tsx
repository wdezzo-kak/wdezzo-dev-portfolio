...
const Projects: React.FC<ProjectsProps> = ({ onInteract, dynamicProjects }) => {
  ...
  const randomizedProjects = useMemo(() => {
    return [...displayProjects].sort(() => Math.random() - 0.5);
  }, [displayProjects]);

  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data === 'closeModal') setActiveDemo(null);
    };
    ...
    <section id="work" className="py-20 border-b-4 border-black dark:border-white bg-white dark:bg-brutal-black transition-colors duration-300">
        ...
        {activeDemo && (
          <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="absolute inset-[10px] sm:inset-[30px] flex flex-col bg-white dark:bg-black border-4 border-black dark:border-white shadow-hard-xl dark:shadow-hard-white-xl z-[2001]"
            ...
            >
              <div className="flex-shrink-0 h-12 bg-brutal-lime border-b-4 border-black dark:border-white flex items-center justify-between px-4 select-none">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono font-bold text-xs sm:text-sm truncate max-w-[120px] sm:max-w-md uppercase text-black">
                    {activeDemo.includes('notfound.html') ? 'SYSTEM_ERROR' : `INTERFACE_ONLINE: ${activeDemo.split('/').pop()}`}
                  </span>
                </div>
                <button 
                  onClick={() => setActiveDemo(null)}
              ...
              <div className="flex-grow bg-white dark:bg-[#1a1a1a] relative w-full h-full overflow-hidden flex flex-col">
                {isLoading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-white dark:bg-black p-4 text-center">
                      <span className="font-mono text-black dark:text-white font-black animate-pulse uppercase tracking-widest text-xs mb-2">VERIFYING_PROJECT_STREAMS...</span>
                      <div className="w-48 h-1 bg-neutral-200 dark:bg-neutral-800 border border-black dark:border-white overflow-hidden">
                          <motion.div 
                            initial={{ x: '-100%' }}
                            animate={{ x: '100%' }}
                            transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}
                            className="w-full h-full bg-brutal-lime"
                          />
                      </div>
                  </div>
                )}
                <iframe 
                  ...
                  className={`w-full h-full relative z-10 border-0 block transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12 md:mb-16">
          <h2 className="text-5xl sm:text-6xl md:text-8xl font-black uppercase tracking-tighter mb-4 text-black dark:text-white leading-none">
            Selected Work
          </h2>
