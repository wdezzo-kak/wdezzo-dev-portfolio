...
const ProjectPreview: React.FC<ProjectPreviewProps> = ({ projects }) => {
  ...
  return (
    <div className="fixed inset-0 bg-neutral-900 flex flex-col font-mono selection:bg-brutal-lime z-[9999] text-white" role="dialog" aria-modal="true" aria-labelledby="simulator-title">
      <header className="h-16 bg-brutal-yellow border-b-4 border-black flex items-center justify-between px-6 z-50 shadow-hard shrink-0 text-black">
        <div className="flex items-center gap-4 text-black">
          <button 
            onClick={() => navigate('/')} 
            className="flex items-center gap-2 bg-black text-white px-3 py-1 text-xs font-black border-2 border-white shadow-hard hover:bg-brutal-red transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-black"
          ...
          <div className="hidden md:flex flex-col text-black">
            ...
            <h1 id="simulator-title" className="text-sm font-black uppercase tracking-tighter truncate max-w-[200px] text-black">{project.title}</h1>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 border-2 border-black bg-white px-3 py-1 font-black text-[10px] shadow-hard text-black" aria-hidden="true">
      ...
      <nav className="bg-neutral-800 py-4 flex justify-center border-b-2 border-white/10 shrink-0" aria-label="Device view controls">
        <div className="flex bg-white p-1 border-4 border-black shadow-hard text-black">
          <button 
            onClick={() => setMode('MOBILE')}
            className={`flex items-center gap-2 px-6 py-2 text-xs font-black uppercase transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brutal-lime ${mode === 'MOBILE' ? 'bg-brutal-lime text-black border-2 border-black -translate-y-1 shadow-none' : 'text-black hover:bg-neutral-100'}`}
            aria-label="Switch to mobile viewport"
            aria-pressed={mode === 'MOBILE'}
            type="button"
          >
            <Smartphone size={16} aria-hidden="true" /> <span className="hidden sm:inline">Mobile</span>
          </button>
          <button 
            onClick={() => setMode('TABLET')}
            className={`flex items-center gap-2 px-6 py-2 text-xs font-black uppercase transition-all border-x-4 border-black focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brutal-lime ${mode === 'TABLET' ? 'bg-brutal-lime text-black border-2 border-black -translate-y-1 shadow-none' : 'text-black hover:bg-neutral-100'}`}
            aria-label="Switch to tablet viewport"
            aria-pressed={mode === 'TABLET'}
            type="button"
          >
            <Tablet size={16} aria-hidden="true" /> <span className="hidden sm:inline">Tablet</span>
          </button>
          <button 
            onClick={() => setMode('LAPTOP')}
            className={`flex items-center gap-2 px-6 py-2 text-xs font-black uppercase transition-all focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brutal-lime ${mode === 'LAPTOP' ? 'bg-brutal-lime text-black border-2 border-black -translate-y-1 shadow-none' : 'text-black hover:bg-neutral-100'}`}
            aria-label="Switch to laptop viewport"
            aria-pressed={mode === 'LAPTOP'}
            type="button"
          >
            <Monitor size={16} aria-hidden="true" /> <span className="hidden sm:inline">Laptop</span>
          </button>
        </div>
      </nav>

      <main className="flex-grow relative overflow-hidden flex flex-col items-center justify-center p-4 sm:p-8 bg-neutral-900/50">
        <motion.div 
          layout
          initial={false}
          animate={{ width: getWidth(), height: '100%' }}
          transition={{ type: "spring", stiffness: 300, damping: 30 }}
          className={`relative bg-white border-4 border-black shadow-hard-xl overflow-hidden flex flex-col transition-all duration-300 ${mode !== 'LAPTOP' ? 'rounded-[32px] border-[12px]' : ''}`}
        >
          {loading && (
      ...
      <footer className="h-8 bg-black text-white border-t border-white/10 flex items-center px-4 justify-between font-mono text-[8px] font-black uppercase tracking-widest opacity-60 shrink-0" aria-hidden="true">
        <span className="text-white">MODE: {mode}_SIMULATION</span>
        <span className="text-white">RESOLUTION: {getWidth()} x FLUID</span>
      </footer>
    </div>
  );
};
